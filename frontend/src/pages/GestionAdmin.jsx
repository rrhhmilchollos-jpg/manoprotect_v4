...
export default function GestionAdmin() {
  ...
  const loadAll = useCallback(async () => {
    setLoading(true);
    try {
      const [st, us, sk, pe, ins, lo] = await Promise.all([
        gFetch('/dashboard/stats'), gFetch('/usuarios'), gFetch('/stock'),
        gFetch('/pedidos'), gFetch('/instalaciones'), gFetch('/logs?limit=50')
      ]);
      setStats(st); setUsuarios(us.usuarios || []); setStock(sk.stock || []);
      setPedidos(pe.pedidos || []); setInstalaciones(ins.instalaciones || []); setLogs(lo.logs || []);
                ...
                {instalaciones.map(inst => (
                  <div key={inst.instalacion_id} className="bg-slate-900 border border-slate-800 rounded-xl p-3" data-testid={`inst-row-${inst.instalacion_id}`}>
                    ...
                    {inst.estado === 'pendiente' && (
                      <button onClick={() => setModal({ type: 'assign_installer', data: inst })} className="mt-2 text-xs bg-amber-600/20 border border-amber-500/30 text-amber-400 px-2.5 py-1.5 rounded-lg">
                        Asignar Instalador
                      </button>
                    )}
                  </div>
